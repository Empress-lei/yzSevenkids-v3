 ...
 * @param second 秒数
 */
function fmtWeekSecond(second){
	return fmtDate(new Date(second*1000),'yyyy-MM-dd E p hh:mm:ss');
}
/**
 * 
 */
...
function patternDate(fmt) {      
    ...
    "d+" : this.getDate(), //日       
    "h+" : this.getHours() > 12 ? ( this.getHours() == 24 ? 0 : this.getHours() - 12 ) : this.getHours(), //小时       
    "H+" : this.getHours(), //小时       
    "m+" : this.getMinutes(), //分       
    "s+" : this.getSeconds(), //秒       
    ...
    if(/(p+)/.test(fmt)){
    		var v=this.getHours() > 12 ? '下午' : '上午';
		fmt=fmt.replace(RegExp.$1, ((RegExp.$1.length>1) ? (RegExp.$1.length>2 ? "星期" : "周") : "")+ v);
    }
    for(var k in o){       
